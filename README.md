...
data/        input CSVs and the generated faculty-level files
  ...
  google_scholar_citations_manual.csv   optional, manually verified citation supplement
...
4. **`code/04_publication_tables.R`** — Produces Tables D1–D4, the publication qualification flow, impact-factor summary statistics, citations by publication year, journals ranked by publication count, publication audits, figures, and the combined publication workbook from the stage 03 outputs and `publications_faculty_doi_updated.csv`.
...
- **Google Scholar citations:** Do not edit `output/publications_enriched.csv`; stage 02 replaces it on each run. Store manually verified values in `data/google_scholar_citations_manual.csv`, with one row per `publication_key` and columns `publication_key`, `google_scholar_citations`, `google_scholar_retrieved_date`, `google_scholar_source_url`, and `google_scholar_match_notes`. Stage 04 uses this supplement as the primary citation measure without combining its counts with OpenAlex. OpenAlex remains the source for structured authorship and affiliation data used in collaboration and interdisciplinarity analysis.
...
- **Remaining self-study tables:** `code/06_remaining_tables.R` produces the faculty snapshot, within-department collaboration, partial CSU-unit and external-partner collaboration, faculty research recognitions, graduate-committee mentoring activity, and student-engagement tables. Unsupported Academic Analytics, multistate-project, faculty-development, and student-engagement fields have schema-valid templates and are documented in `EXTERNAL-DATA-REQUESTS.md`.
- **Definitions for Tables A, B, D, E, F, and G:** see `TABLES-CODEBOOK.md` for source lineage, counting rules, and limitations.
